 ...
 ** Class: CS 344
 ** Date: 2017.07.23
 ** Program: goodwiel.buildrooms
 ** Description: Creates a "graph" represnting connected rooms for a
 **     simple adventure game. Program demonstrates creating
 **     direcotries and files for text storage.
 *********************************************************************/
...
int main(){
    ...
    int i = 0;
    //int the cstrings with empty memory
    ...
    //start ranomdizing the rooms
    int roomCount = 0;
    int randomIndx = 0; // random indx for inserting
    
    while(roomCount < MAXRANDOMROOMS){
        //select a number between 0 to 9, that refers to the index of the room array
        randomIndx = rand() % MAXROOMS;
        //insert that room into the random room array if it has not been used
        if(roomNames[randomIndx][0] != '\0'){
            strcpy(theRooms[roomCount].roomName, roomNames[randomIndx]);
            //increment the counter  by 1, do this until 7 rooms have been added
            memset(roomNames[randomIndx], '\0', MAXCHARSIZE);
            ++roomCount;
        }
    }

    //select the start location
    randomIndx = rand() % MAXRANDOMROOMS;
    strcpy(theRooms[randomIndx].roomType, START_ROOM);
    
    
    int assignEND = 0;
    while(assignEND != 1){
        randomIndx = rand() % MAXRANDOMROOMS;
        //any room that does not begin with S is fine
        if(theRooms[randomIndx].roomType[0] != 'S'){
            strcpy(theRooms[randomIndx].roomType, END_ROOM);
            assignEND = 1;
        }
    }
    
    //set all other rooms to mid rooms
    for(i = 0; i < MAXRANDOMROOMS; ++i){
        if(theRooms[i].roomType[0] == '\0'){
            strcpy(theRooms[i].roomType, MID_ROOM);
        }
    }
    

    //randomize connections
    while(isGraphFull(theRooms) != 1){
        addRandomConnection(theRooms);
    }
    //print stuff out and the then write it
    //printRooms(theRooms);
    writeFile(theRooms);//
    return 0;
}
...
void displayChoices(struct room* aRoom){
    ...
    int i;
    //print out choices in correct format
